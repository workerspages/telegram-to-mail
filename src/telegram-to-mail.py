...
from telethon.errors import FloodWaitError  # ★ 已保留
...
# --- ★ 新增：异步延迟删除消息任务 ★ ---
async def delayed_delete_task(client, chat_id, message_id, delay_seconds):
    """
    在指定延迟时间后删除消息。
    """
    if delay_seconds <= 0:
        return
    print(f"[Auto-Delete] Message {message_id} in {chat_id} scheduled for deletion in {delay_seconds}s.")
    await asyncio.sleep(delay_seconds)
    try:
        await client.delete_messages(chat_id, [message_id])
        print(f"[Auto-Delete] Successfully deleted message {message_id} in {chat_id}.")
    except Exception as e:
...
async def handle_message(event):
        ...
        if target_chat:
            try:
                ...
                async def do_forward():
                    ...
                    print(f"[Forward] Message forwarded from {group_name} to {target_chat}")
    ...
    if group.get('auto_delete_enabled', False):
        try:
            ...
            asyncio.create_task(delayed_delete_task(client, event.chat_id, message_id, delete_delay))
        except (ValueError, TypeError):
            pass
                        ...
                        async with aiohttp.ClientSession() as session:
                            async with session.get(url, headers=headers, timeout=10) as resp:
                                if resp.status == 200:
                                    ...
                                    if msgs:
                                        latest_text = msgs[-1].get_text(separator='\n').strip()
                                        msg_hash = hashlib.md5(latest_text.encode('utf-8')).hexdigest()
                                        if msg_hash != scraper_state.get(username):
                                            print(f"[Scraper] New message found in @{username}")
                                            scraper_state[username] = msg_hash
                                            save_scraper_state(scraper_state)
                                            subject = f"【TG订阅】{channel.get('name') or f'@{username}'} 更新"
                                            await process_notifications(config, channel.get('notifiers', []), subject, f"{latest_text}\n\n(来源: Web Preview)")
                    except Exception as e:
        ...
        try:
            ...
            if config:
                ...
                for idx, task in enumerate(tasks):
                    ...
                    target = task.get('target'); msg = task.get('message')
                    ...
                    t_end_str = task.get('time_end') or "08:00"
                    try:
                        ...
                        if target_key not in daily_random_targets:
                            ...
                            random_dt = datetime.fromtimestamp(random.uniform(time_min_ts, time_max_ts)) if time_max_ts > time_min_ts else now
                            daily_random_targets[target_key] = random_dt
                        
                        if now >= daily_random_targets[target_key]:
                            ...
                            state[task_sig] = today_str; save_state(state)
...
async def main():
    ...
    if not api_id or not api_hash or not os.path.exists(SESSION_NAME):
        print("Telegram Client skipped. API info or Session file missing."); while True: await asyncio.sleep(3600)
    try:
        client = TelegramClient(SESSION_NAME, int(api_id), api_hash)
        @client.on(events.NewMessage)
        async def event_handler(event): await handle_message(event)
        await client.start()
